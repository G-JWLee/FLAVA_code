...
if __name__ == '__main__':
    ...
    retrieve_dir = os.path.join(base_path, 'FLAVA_code/experiments/tblogs/cav_base_vggsound_pretrain/cav_base_vggsound_pretrain')
    ...
    for buffer_size in buffer_sizes:
        ...
        for method in methods:
            ...
            for task in tasks:
                ...
                new_task_df = pd.DataFrame({'methods': [method_name_dict[method]] * len(seeds),
                                            f'{task}_avg_acc': acc_avg_list, f'{task}_avg_fgt': forget_avg_list})
        ...
        for task in tasks:
            summarize_df = pd.DataFrame(task_dfs[task])
            bar_plot(summarize_df, task_titles[task] + ' Fgt', f'{task}_avg_fgt', palettes)
            bar_plot(summarize_df, task_titles[task] + ' Avg', f'{task}_avg_acc', palettes)