...
def dot_plot(df, task, task_name, palettes, metric):
    ...
    plt.yticks(np.arange(round(minimum,1)-0.02, round(maximum,1)+0.02, 0.02), fontsize=28)

    plt.ylim([minimum-0.01, maximum+0.03])
...
if __name__=='__main__':
    ...
    method_dir = os.path.join(base_path, 'FLAVA_code/experiments/tblogs/cav_base_vggsound_pretrain/cav_base_vggsound_pretrain')
    ...
    dot_plot(modality_dfs, "VGGSound Modality Gap", "modality_gap",
             palettes, 'Distance')
